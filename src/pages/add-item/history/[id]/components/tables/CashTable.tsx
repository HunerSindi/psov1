...
interface Props {
    ...
    /** When true (e.g. admin), show cost and profit columns */
    showCostProfit?: boolean;
}

export default function CashTable({ items, showCostProfit }: Props) {
    const { t } = useSettings();
            ...
            <table className="w-full border-collapse">
                ...
                <thead className="border border-black bg-gray-100">
                    <tr>
                        ...
                        {showCostProfit && (
                            <>
                                <th className="p-2 text-right text-[10px] uppercase w-20 border border-black">
                ...
                <tbody className="text-[10px]">
                    {items.map((item, idx) => (
                        <tr key={idx} className="border border-black">
                            ...
                            {showCostProfit && (
                                <>
                                    <td className="p-2 border border-black text-right font-mono">{(item.cost ?? 0).toLocaleString()}</td>
                                    <td className="p-2 border border-black text-right font-mono text-green-700">{(item.profit ?? 0).toLocaleString()}</td>
                                </>
                            )}
                        </tr>
                    ))}
