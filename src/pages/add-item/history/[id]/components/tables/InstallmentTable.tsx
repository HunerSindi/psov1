...
export default function InstallmentTable({ items, showCostProfit }: Props) {
    const { t } = useSettings();
    return (
            ...
            <table className="w-full text-left border-collapse">
                ...
                <thead className="bg-blue-600 border-b border-blue-800 text-xs uppercase font-bold text-white">
                    <tr>
                        ...
                        {showCostProfit && (
                            <>
                                <th className="p-3 border-r border-blue-500 text-right w-28">{t("sales_history.detail.cost")}</th>
                                <th className="p-3 text-right w-28">{t("sales_history.detail.profit")}</th>
                            </>
                        )}
                    </tr>
                </thead>
                <tbody className="divide-y divide-blue-100 text-sm">
                    {items.map((item, idx) => (
                        <tr key={item.id} className="hover:bg-blue-50 transition-colors">
                            ...
                            {showCostProfit && (
                                <>
                                    <td className="p-3 border-r border-blue-100 text-right font-mono">{(item.cost ?? 0).toLocaleString()}</td>
                                    <td className="p-3 text-right font-mono text-green-700">{(item.profit ?? 0).toLocaleString()}</td>
                                </>
                            )}
                        </tr>
                    ))}
