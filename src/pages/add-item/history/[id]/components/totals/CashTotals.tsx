...
interface Props {
    ...
    /** When true (admin), show total cost and profit */
    showCostProfit?: boolean;
    totalCost?: number;
    totalProfit?: number;
}

export default function CashTotals({ receipt, showCostProfit, totalCost, totalProfit }: Props) {
    const { t } = useSettings();
                ...
                <table className="w-full  ">
                    <tbody>
                        ...
                        {showCostProfit && totalCost != null && (
                            <tr className="text-xs">
                                <td className="text-right p-1 font-bold">{t("sales_history.detail.total_cost")}:</td>
                                <td className="text-right p-1 w-32 font-mono">{(totalCost ?? 0).toLocaleString()}</td>
                            </tr>
                        )}
                        {showCostProfit && totalProfit != null && (
                            <tr className="text-xs text-green-700">
                                <td className="text-right p-1 font-bold">{t("sales_history.detail.total_profit")}:</td>
                                <td className="text-right p-1 w-32 font-mono font-bold">{(totalProfit ?? 0).toLocaleString()}</td>
                            </tr>
                        )}
                    </tbody>
                </table>
