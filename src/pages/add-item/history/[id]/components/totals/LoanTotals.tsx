...
export default function LoanTotals({ receipt, showCostProfit, totalCost, totalProfit }: Props) {
    const { t } = useSettings();
    ...
    return (
        <div className="flex justify-end font-sans text-black">
            <div className="w-80 border-2 border-black bg-gray-50 text-sm shadow-sm">
                ...
                {showCostProfit && (totalCost != null || totalProfit != null) && (
                    <>
                        {totalCost != null && (
                            <div className="p-2 border-t border-gray-300 flex justify-between text-sm">
                                <span>{t("sales_history.detail.total_cost")}:</span>
                                <span className="font-mono">{(totalCost ?? 0).toLocaleString()}</span>
                            </div>
                        )}
                        {totalProfit != null && (
                            <div className="p-2 border-t border-gray-300 flex justify-between text-sm text-green-700 font-bold">
